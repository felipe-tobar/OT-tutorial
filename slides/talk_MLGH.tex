...
\begin{itemize}
\item[$\bullet$] $\alpha_i$ = price for \textbf{loading} a kilo of grapes at place $\er{x_i}$ (no matter to which plant it goes)
\item[$\bullet$] $\beta_j$ = price for \textbf{unloading} a kilo of grapes at place $\eb{y_j}$ (no matter from which vineyard it comes from)
\end{itemize}
...
\visible<2>{\item[$\bullet$] A feasible deal for the vendor requires that  $\alpha_i+\beta_j\leq C_{ij}$.}
